...
public class ContaMediator {
	...
	private static final String MSG_CORRENTISTA_NAO_ENCONTRADO = "Correntista n�o cadastrado";
	...
	public StatusValidacaoConta incluir(Conta conta) throws ExcecaoNegocio, ExcecaoObjetoJaExistente {
		...
		if (status.isValido()) {
			try{
				...
				System.out.println(MSG_CONTA_NAO_INCLUIDA);
			}
			try {
	...
	public StatusValidacaoConta alterar(Conta conta) {
		...
		if (status.isValido()) {
			try {
				...
				System.out.println(MSG_CONTA_NAO_ENCONTRADA);
			}
			try {
	...
	public boolean encerrarConta(long numero) {
		...
		try {
			validarAlterarStatusConta(numero, statusProibidos, StatusConta.ENCERRADA);
		} catch (ExcecaoObjetoNaoExistente e) {
			e.printStackTrace();
		}
		return false;				
	...
	public boolean bloquearConta(long numero) {
		...
		try {
			validarAlterarStatusConta(numero, statusProibidos, StatusConta.BLOQUEADA);
		} catch (ExcecaoObjetoNaoExistente e) {
			e.printStackTrace();
		}
		return false;		
	...
	public boolean desbloquearConta(long numero) {
		...
		try {
			validarAlterarStatusConta(numero, statusProibidos, StatusConta.ATIVA);
		} catch (ExcecaoObjetoNaoExistente e) {
			e.printStackTrace();
		}
		return false;
